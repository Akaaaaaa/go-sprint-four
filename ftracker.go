...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	//    (duration * minInH)
	burnCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(secm, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * (duration * minInH)
	return burnCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// 1. Вычисляем среднюю скорость (м/с)
	// Дистанция = длина бассейна * количество дорожек
	// Время переводим в часы (если duration в часах, то оставляем)
	distance := float64(lengthPool) * float64(countPool) / mInKm // дистанция в км
	speed := distance / duration                                 // скорость в км/ч

	// 2. Рассчитываем калории
	// Формула: (СредняяСкорость + Смещение) * Множитель * Вес * Время
	// Для плавания обычно: (speed + 1.1) * 2 * weight * duration
